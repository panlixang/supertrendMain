# -*- coding: utf-8 -*-
"""MU-USDT-SWAP 最近一个月参数寻优。

与 _sweep_mu.py 同口径（分阶段贪心：params → ER → 分数拦截），
只是回测窗口截为最近 BARS=744 根 1h（约 31 天）。
数据用本地 _live_data/MU.json（_refresh_data.py 刷新到最新）；
bias 周期保留到窗口末的全部历史（_cbtf_at 按 ts 过滤，无未来泄漏）。
出场规则固定为线上 MU 增强档，与 _sweep_mu.py 一致。

注意：BASE_CFG / 基线取自线上 /api/trade/symbols 的 MU 真实配置
（2026-09-02 查询：periods=18, multiplier=3.0, er 0.2/0.15/0.25, scoring 45/45/35）。
"""
...
SYM = "MU-USDT-SWAP"
TF = "1h"
BARS = 744          # 最近一个月（31 天 × 24h）
BIAS_TFS = ["15m", "1h", "4h", "1d"]

# 线上 MU 增强档出场规则（固定，不参与寻优）
ER = dict(
...
def main():
    ...
    if len(all_1h) < BARS + 5:
        print(f"1h 数据不足: {len(all_1h)}", flush=True)
        return
    end_ts = all_1h[-1]["ts"]
    candles = all_1h[-BARS:]
    # bias 周期保留到窗口末的全部历史（无未来数据）
    ...
    for t in BIAS_TFS:
        ...
        if arr:
            cbtf[t] = arr
    print(f"窗口: {ts_fmt(candles[0]['ts'])} ~ {ts_fmt(end_ts)} "
          f"({len(candles)} 根 1h)", flush=True)
    for t, arr in cbtf.items():
        print(f"  bias {t}: {len(arr)} 根", flush=True)

    # 基线：线上当前配置 18×3.0 / er 0.2/0.15/0.25 / scoring 45/45/35 m0
    base = one({"periods": 18, "multiplier": 3.0, "src": "hl2", "change_atr": True,
    ...
    for per in periods:
        for mul in mults:
            ...
            if r:
                r.update(params=f"{per}×{int(mul)}", er="0.2/0.15/0.25",
                         score="d:45/45/35 m0")
                rows1.append(r)
    ...
    for base_ in top2:
        ...
        for m_score in (0, 1, 2):
            for full in (45.0, 60.0, 80.0):
                for half in (40.0, 45.0, 60.0):
                    for alert in (30.0, 35.0, 40.0):
                        r = one(p, make_cfg(er_min=er_min, er_weak_min=er_weak,
    ...
    out = os.path.join(os.path.dirname(__file__), "_sweep_mu_1m.json")
    with open(out, "w", encoding="utf-8") as f:
        json.dump({"window": [ts_fmt(candles[0]["ts"]), ts_fmt(end_ts)],
