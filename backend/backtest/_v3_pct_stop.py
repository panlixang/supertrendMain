# -*- coding: utf-8 -*-
"""正确基线 = 三级止盈(TP1 1%平30%/TP2 2%平40%/TP3 3.5%全平) + V3.
补测真实硬止损 sl_mode="pct" 2/3/4%; 基准对照 sl_mode="st". 全长 + 2026."""
...
def report(name,er):
    ...
    y26=[(ts,tmap[ts]["pnl_pct"]) for _,ts,_ in sel if ts>=T2026*1000 and ts in tmap]
    print("\n== %s =="%name); metrics(full,"全长"); metrics(y26,"2026")

# 三级止盈 + V3; 仅改 sl_mode / sl_pct
def etp(sl_mode, sl_pct):
    return EnhancedExitRules(enabled=True,trail_with_st=True,sl_mode=sl_mode,
                             move_sl_to_entry=True,sl_pct=sl_pct,
                             tp1_pct=1.0,tp1_ratio=30.0,tp2_pct=2.0,tp2_ratio=40.0,
                             tp3_pct=3.5,tp3_ratio=100.0)
report("【基准 三级+st】sl_mode=st 兜底2%", etp("st",2.0))
for pct in (2.0,3.0,4.0):
    report("【三级+硬止损 pct %d%%】"%int(pct), etp("pct",pct))
