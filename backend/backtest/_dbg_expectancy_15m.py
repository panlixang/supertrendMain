# -*- coding: utf-8 -*-
"""
15m 数据集正期望值检验 (同 1h 口径):
基线(全做) / 线性打分前20% / 单边规则ATR_pct阈值 / 高波动反向对照
含: 扣费后净收益 + t检验 + bootstrap置信区间 + 收益集中度
"""
import csv
import math
import random
...
random.seed(42)
PATH = r"d:\个人项目代码\supertrendMain\backend\backtest\st_signals_15m.csv"
...
print(f"15m 数据集: 总 {n}  训练(前70%) {len(train)}  检验(后30%) {len(test)}")
print(f"训练区间 {train[0]['time']} ~ {train[-1]['time']}")
print(f"检验区间 {test[0]['time']} ~ {test[-1]['time']}\n")
