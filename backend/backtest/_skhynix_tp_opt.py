"""SKHYNIX-USDT-SWAP 止盈止损档位寻优。

信号侧完全对齐线上（47.84.106.154:5174）：1h 周期、9×3.0、ER 0.11/0.12/0.12/0.3、
打分制 70/70/60 动态阈值、A/B min_score=2、过滤器全关、10U×10x。
只扫 exit_rules 的三档止盈幅度 tp1/tp2/tp3，其余字段保持线上原样。
"""
...
LIVE2 = "http://47.84.106.154:5174/api/trade/symbols"
GATE_TF = "1h"
BARS_N = BARS["1h"]
MIN_TRADES = 5

TP1S = [0.5, 0.8, 1.0, 1.2, 1.5, 1.8, 2.0, 2.5]
TP2S = [1.5, 1.8, 2.0, 2.5, 3.0, 3.5]
...
def main():
    ...
    out = {
        "symbol": SYMBOL, "tf": GATE_TF, "live": LIVE2,
        "start": ts_fmt(start), "end": ts_fmt(end), "bars": len(candles),
        "filters": "线上 SKHYNIX：ER 0.11/0.12/0.12/0.3 + 打分制 70/70/60 动态 + A/B min_score=2 "
                   "+ 过滤器全关 + 10U×10x 1h（max_loss 关）",
