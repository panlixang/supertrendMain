# -*- coding: utf-8 -*-
"""SPCX-USDT-SWAP 参数寻优：periods×multiplier、ER 档位、分数拦截。
出场规则固定为线上 SPCX 增强档（tp1=1.2/30, tp2=2.0/40, tp3=3.5/100 reverse,
sl=st 3.0, max_loss=5.0）。quick 关闭（线上 quick_enabled=False）。
分阶段贪心。

支持历史窗口 OOS 验证：
  python _sweep_spcx.py --start 2026-05-07 --end 2026-07-01
不传参数 = 拉取最新 4500 根 1h（品种 2026-05-07 上线，实际约 2800 根）。"""
...
# 线上 SPCX 增强档出场规则（固定，不参与寻优）
ER = dict(
    enabled=True, tp1_pct=1.2, tp1_ratio=30.0, tp2_pct=2.0, tp2_ratio=40.0,
    ...
    protect_trail_pct=0.8, max_loss_enabled=True, max_loss_pct=5.0,
)
...
BASE_CFG = dict(
    ...
    scoring_half_threshold=45.0, scoring_alert_threshold=40.0,
...
def main():
    ...
    # 基线：线上当前配置 8×5.0 / er 0.12/0.12/0.3 / scoring 45/45/40 m0
    base = one({"periods": 8, "multiplier": 5.0, "src": "hl2", "change_atr": True,
    ...
    periods = list(range(4, 21, 2))        # 4,6,...,20（含当前8）
    ...
    for base_ in top2:
        ...
        for m_score in (0, 1, 2):
            for full in (35.0, 45.0, 60.0, 80.0):
                for half in (30.0, 45.0, 60.0):
                    for alert in (30.0, 40.0, 50.0):
                        r = one(p, make_cfg(er_min=er_min, er_weak_min=er_weak,
    ...
    out = os.path.join(os.path.dirname(__file__), f"_sweep_spcx{tag}.json")
