"""XAU-USDT-SWAP 1h 联合寻优：闸门 + 超趋。

XAU 不在线上交易列表，出场对齐现盘模板：
...
def main():
    sym = gold_sym()
    print(f"### fetch XAU {GATE_TF} ###", flush=True)
    ...
    out = run_symbol("XAU", sym, candles, cbtf)
    out["symbol"] = SYMBOL
    out["exit"] = "tp1 1%/30 + tp2 2%/40 + tp3 reverse_signal + ST trail + max_loss 3%"
    path = os.path.join(os.path.dirname(__file__), "_xau_joint_opt_1h.json")
    with open(path, "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    print(f"\nWrote {path}", flush=True)
    slim = {"live": out.get("live"), "best": out.get("best"),
