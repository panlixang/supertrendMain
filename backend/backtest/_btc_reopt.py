"""BTC 当前线上口径（er_min=0.15 / quick off / 纯分数制）ST 参数寻优。

数据：本地 _live_data/BTC.json（1h，含 bias 周期）。
网格：periods 7..19 × mult 2.0..6.0 × score 35..70。
...
ER_MIN = 0.12  # 方案A：弱档并入正常档，全走正常档规则
...
def main():
    ...
    print(f"=== BTC {GATE_TF} 方案A口径(er_min={ER_MIN}, quick=off, 弱档并入正常档) 寻优 ===", flush=True)
    ...
    out_path = Path(__file__).parent / "_btc_reopt.json"
