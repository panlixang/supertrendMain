# -*- coding: utf-8 -*-
"""SNDK v1 vs v2 A/B 验证（43 对照机口径，沿用 _score_v2_multi 框架）。

SNDK 在 43：ST 13x3.0、ER 0.1/0.12、v1 @ full/half=70 alert=55（conv≈0.39，
engine_advisor 判 V1）。本实验回答：SNDK 上 v2 是否真的更差，v1 现行阈值是否最优。
  A       = v1 现行（43 线上配置）
  B_same  = v2 同阈值（引擎差异本身）
  grid    = v2 阈值扫描 [40..75]（B* 模式）
  E       = B* vs A 前后半段
写 _score_v2_sndk.json。
"""
...
def main():
    t0 = time.time()
    s43 = next(s for s in _get(LIVE43)["symbols"] if s["symbol"] == "SNDK-USDT-SWAP")
    print("== 43 SNDK ==", {k: s43.get(k) for k in (
    ...
    print(f"ST {p.get('periods')}x{p.get('multiplier')}  "
          f"ex.max_loss={ex.max_loss_enabled}", flush=True)
    ...
    out = {
        ...
        "A_v1_now": run_key("A(v1@70)", cfgA),
        "B_v2_same": run_key("B(v2@70)", cfgB),
    }
    ...
    with open(os.path.join(DATA, "_score_v2_sndk.json"), "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=1, default=str)
    print(f"\nWrote _score_v2_sndk.json | 总耗时 {time.time() - t0:.0f}s", flush=True)
