# -*- coding: utf-8 -*-
"""三组出口 × 真实硬止损(sl_mode=pct 2/3/4%) + ST基线对照; 报2025 & 2026.
同一 V3 信号集 550. ①三挡tp3=3.5%pct全平 ②单档1.5%/70% ③三挡tp3=reverse."""
...
T2025 = calendar.timegm((2025,1,1,0,0,0)); T2026 = calendar.timegm((2026,1,1,0,0,0))
sel25=[t for t in sel if t[1]>=T2025*1000]; sel26=[t for t in sel if t[1]>=T2026*1000]
print("V3过滤:%d  2025:%d  2026:%d"%(len(sel),len(sel25),len(sel26)))
...
def report(name,er):
    ...
    y25=[(ts,tmap[ts]["pnl_pct"]) for _,ts,_ in sel25 if ts in tmap]
    y26=[(ts,tmap[ts]["pnl_pct"]) for _,ts,_ in sel26 if ts in tmap]
    print("\n== %s =="%name)
    metrics(y25,"2025"); metrics(y26,"2026")

def cfgA(mode,pct):  # 三挡tp3=3.5%pct全平
    return EnhancedExitRules(enabled=True,trail_with_st=True,sl_mode=mode,move_sl_to_entry=True,
        sl_pct=pct,tp1_pct=1.0,tp1_ratio=30.0,tp2_pct=2.0,tp2_ratio=40.0,
        tp3_pct=3.5,tp3_ratio=100.0,tp3_mode="pct")
def cfgB(mode,pct):  # 单档1.5%/70%
    return ExitRules(tp1_pct=1.5,tp1_ratio=70.0,sl_mode=mode,sl_pct=pct,
        move_sl_to_entry=True,trail_with_st=True,reverse_close=False)
def cfgC(mode,pct):  # 三挡tp3=reverse_signal
    return EnhancedExitRules(enabled=True,trail_with_st=True,sl_mode=mode,move_sl_to_entry=True,
        sl_pct=pct,tp1_pct=1.0,tp1_ratio=30.0,tp2_pct=2.0,tp2_ratio=40.0,
        tp3_pct=3.5,tp3_ratio=100.0,tp3_mode="reverse_signal")

for tag,fn in (("①三挡tp3=3.5%pct全平",cfgA),("②单档1.5%/70%",cfgB),("③三挡tp3反向",cfgC)):
    report("%s [ST基线]"%tag, fn("st",2.0))
    for pct in (2.0,3.0,4.0):
        report("%s [硬止损pct%d%%]"%(tag,int(pct)), fn("pct",pct))
