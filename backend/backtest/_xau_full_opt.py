# -*- coding: utf-8 -*-
"""XAU-USDT-SWAP 全量下单配置联合寻优（坐标下降，分阶段）。
...
def run_cfg(candles_map, gate_tf, params, er, score, exit_r, q_r):
    ...
    r = run_backtest(
        gate_c, params, init_cash=100.0, fee_rate=0.0005, allow_short=True,
...
def main():
    ...
    def _run_on(cbtf):
        gc = cbtf[GATE_TF]
        return run_backtest(gc, st_params(PST, PMULT), init_cash=100.0, fee_rate=0.0005,
    ...
    path = os.path.join(os.path.dirname(__file__), "_xau_full_opt.json")
