"""15m 信号 + 4h MA30 方向门 + 线上评分/TP-SL (OKX / 10x)。

同窗口 4 组合(信号周期=15m):
  ① 11×4 无门      ② 11×4 + MA30 门
  ③ 10×3 无门      ④ 10×3 + MA30 门
allow_tfs 覆盖为 ["15m"] 以对齐"线上只跑该周期"的口径。
"""
...
def main():
    ...
    gate_tf = "15m"
    bars = {"15m": 18000, "1h": 4500, "4h": 1200, "1d": 220}

    c15 = L.fetch_candles(sym["symbol"], "15m", bars["15m"])
    cbtf = {"15m": c15}
    for tf in ("1h", "4h", "1d"):
        ex = L.fetch_candles(sym["symbol"], tf, bars[tf])
        if ex:
            cbtf[tf] = ex
    c4h = cbtf["4h"]
    trend_align = build_trend_align(c4h, c15)

    cfg = L.trade_cfg(sym)
    cfg.allow_tfs = ["15m"]   # 对齐"线上只跑该周期"

    print(f"\n=== 窗口 {ts_fmt(c15[0]['ts'])} ~ {ts_fmt(c15[-1]['ts'])}  "
          f"15m {len(c15)} 根 / 10x / 线上评分+TP-SL ===\n")
    ...
    for label, p, ta in configs:
        r = run_backtest(
            c15, p, init_cash=100.0, fee_rate=0.0005, allow_short=True,
        ...
        results.append(row)
        print(f"  {label:<14} 交易 {row['trades']:>4}  净利润 {pnl:>7.2f}U  "
              f"ROI {row['margin_roi_pct']:>7.1f}%  胜率 {row['win_rate']:>5.1f}%  "
              f"PF {row['profit_factor']:>5.2f}  回撤 {row['max_dd_pct']:>5.2f}%  "
              f"评分拦 {row['er_blocked']} 方向拦 {row['align_blocked']}")

    print(f"\n  买入持有(基准): {results[0]['hold_pct']}%")
    out_path = os.path.join(os.path.dirname(__file__), "_live_cfg_btc_15m.json")
