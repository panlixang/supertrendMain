"""SNDK-USDT-SWAP 15m / 1h：关掉组合过滤器，只按 0-100 置信度打分开仓。
...
def sndk_sym(live: dict) -> dict:
    for s in live["symbols"]:
        if "SNDK" in s["symbol"]:
            return copy.deepcopy(s)
    raise RuntimeError("live symbols 里没有 SNDK")
...
def main():
    ...
    out = {"symbol": sym["symbol"], "filters": "仅0-100打分，无ATR/区间/MTF/ADX/等级/强度硬闸",
           "exit_rules": "线上SNDK三级止盈", "by_tf": {}}

    for gate_tf in GATE_TFS:
        bars = BARS.get(gate_tf, 4500)
        print(f"\n=== SNDK {gate_tf} score-only ===", flush=True)
    ...
    path = os.path.join(os.path.dirname(__file__), "_sndk_score_opt.json")
