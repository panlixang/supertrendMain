"""按线上 /api/trade/symbols 配置回测 BTC，但把 SuperTrend 覆盖成 (10,3.0)。

其余（评分闸门 full/half=50、alert=55、ER/区间过滤、TP/SL 出场、10x、
固定保证金 5U）完全沿用线上配置，用于和 _live_cfg_backtest.json(11×4) 对照。
"""
...
# 覆盖的 ST 参数（仅 BTC 生效）
OVERRIDE_ST = {"periods": 10, "multiplier": 3.0, "src": "hl2", "change_atr": True}
...
def main():
    ...
    symbols = [s for s in symbols if "BTC" in s["symbol"].upper()]
    if not symbols:
        print("无 BTC 品种", flush=True)
        return
    sym = symbols[0]

    gate_tf = sym["allow_tfs"][0]
    bars = BARS.get(gate_tf, 4500)
    print(f"\n=== BTC {gate_tf} ST(10,3.0) + 线上评分/TP-SL  ===", flush=True)
    ...
    p = dict(OVERRIDE_ST)  # 覆盖 ST 为 10×3，其余沿用线上
    r = run_backtest(
        candles, p,
    ...
    out_path = os.path.join(os.path.dirname(__file__), "_live_cfg_btc_103.json")
