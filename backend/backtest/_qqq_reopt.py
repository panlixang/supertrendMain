"""QQQ（OKX 永续，未上线）ST 参数寻优。

数据：本地 _live_data/QQQ.json（1h 4273 根 + bias 周期）。
口径：纯分数制，er_min=0.12（方案A，弱档并入正常档），quick off。
默认规则：exit_rules 用线上 BTC 模板，margin=10U、leverage=10x、er_hide_below=0.1。
网格：periods 5..19 × mult 1.5..6.0 × score 35..70。
"""
...
# QQQ 默认配置（未上线，用 BTC 线上规则作模板）
QQQ_SYM = {
    "symbol": "QQQ-USDT-SWAP",
    "params": {"periods": 10, "multiplier": 3.0, "src": "hl2", "change_atr": True,
               "fast_len": 20, "slow_len": 50, "ma_type": "EMA"},
    "exit_rules": {
        "enabled": True, "tp1_pct": 1.0, "tp1_ratio": 30.0,
        "move_sl_to_entry": True, "sl_mode": "st", "sl_pct": 3.0,
        "trail_with_st": True, "tp2_pct": 2.0, "tp2_ratio": 40.0,
...
def load_cbtf() -> dict:
    with open(DATA_DIR / "QQQ.json", "r") as f:
        data = json.load(f)
...
def main():
    sym = copy.deepcopy(QQQ_SYM)
    cbtf = load_cbtf()
    candles = cbtf[GATE_TF]
    print(f"=== QQQ {GATE_TF} 方案A口径(er_min={ER_MIN}, quick=off, 弱档并入正常档) 寻优 ===", flush=True)
    ...
    if cur:
        print(f"\n  基准 10×3.0 score≥45: {cur['pnl_u']}U dd={cur['max_dd_pct']}% trades={cur['trades']} "
    ...
    out_path = Path(__file__).parent / "_qqq_reopt.json"
