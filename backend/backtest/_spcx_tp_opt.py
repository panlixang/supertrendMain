"""SPCX-USDT-SWAP 止盈止损档位寻优。

信号侧完全对齐线上：1h 周期、8×5.0、ER 0.12/0.12/0.3、打分制 45/45/40
动态阈值、A/B/C min_score=0、10U×10x、max_loss 5%。
...
def main():
    ...
    out = {
        ...
        "filters": "线上 SPCX：ER 0.12 + 打分制 45/45/40 动态 + A/B/C min_score=0 + 10U×10x 1h max_loss5%",
        "current": {**s0, "tp": [cur["tp1_pct"], cur["tp2_pct"], cur["tp3_pct"]]},
        "recommend": rec,
        "top8": top,
        "all": rows,
    }
    path = os.path.join(os.path.dirname(__file__), "_spcx_tp_opt.json")
