# -*- coding: utf-8 -*-
"""V3 + 三挡止盈 在 2026 BTC 1h 信号上的回测收益(真引擎 run_backtest)."""
...
variants={
 "三挡止盈+STtrailing(TP3=+3.5%全平)": EnhancedExitRules(enabled=True,trail_with_st=True,sl_mode="st",move_sl_to_entry=True,sl_pct=2.0),
 "三挡止盈+TP3=反向信号(大赢家放行)": EnhancedExitRules(enabled=True,trail_with_st=True,sl_mode="st",move_sl_to_entry=True,sl_pct=2.0,tp3_mode="reverse_signal"),
 "纯三挡止盈(关trailing)": EnhancedExitRules(enabled=True,trail_with_st=False,sl_mode="st",move_sl_to_entry=True,sl_pct=2.0),
}
def metrics(pairs,label):
    ...
    print("  [%s] 笔%3d 复利%7.1f%% 胜率%4.1f%% PF=%4.2f DD%5.1f%% 红字%d 均%5.2f%%" % (
        label,n_,(eq-1)*100,len(wins)/n_*100,pf,mdd/pk*100,red,sum(pn)/n_))
