# -*- coding: utf-8 -*-
"""SK Hynix(海力士) 1h 2026：V3 过滤 + 单档 1.5%/70%（ST基线 & 3%硬止损）。
OKX 代币化股票对海力士的符号不确定，先探测候选，再跑回测。
"""
import sys, calendar
...
CANDIDATES = ["SKHYNIX-USDT-SWAP", "000660-USDT-SWAP", "SKH-USDT-SWAP", "SKHYNIX-USDT-SWAP"]
...
for cand in CANDIDATES:
    try:
        probe = to_dicts(fetch_candles("1h", limit=10, symbol=cand))
    except Exception as e:
        probe = []; print("  [%s] 探测异常: %s" % (cand, e))
    print("  探测 %s -> %d 根" % (cand, len(probe)))
    if len(probe) > 0:
        SYM = cand; break

if SYM is None:
    print("[skhynix] 候选符号都拉不到数据，OKX 可能未上架海力士代币化永续。")
    raise SystemExit(0)
print("采用符号:", SYM)
